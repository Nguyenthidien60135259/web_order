...
class ProductController extends Controller
{
    public function listProduct()
    {
        if(Session::has('email')=='admin@gmail.com'){
            $response = Http::get("http://api.trasuanhatrang.cf/api/products_list");
    ...
    {
        $api_response = Http::post("http://api.trasuanhatrang.cf/api/products_delete/".$request->id);
    ...
    public function createProduct(Request $request)
    {
        ...
        {
            $api_response = Http::get("http://api.trasuanhatrang.cf/api/products_list");
    ...
    public function saveCreateProduct(Request $request)
    {
        ...
        CURLOPT_URL => 'http://api.trasuanhatrang.cf/api/products_create',
    ...
    function updateProduct(Request $request)
    {
        ...
        {
            $id = $request->id;
            $api_response=Http::get("http://api.trasuanhatrang.cf/api/products_show/".$id);
            $data = json_decode($api_response);
            $product = $data->data->product;
            $category = $data->data->category;
            $size = $data->data->size;
    ...
    function saveUpdateProduct(Request $request)
    {
        $curl = curl_init();
        curl_setopt_array($curl, array(
        CURLOPT_URL => 'http://api.trasuanhatrang.cf/api/products_update/'.$request->id,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
