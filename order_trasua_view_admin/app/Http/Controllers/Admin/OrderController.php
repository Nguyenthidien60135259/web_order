...
class OrderController extends Controller
{
    function orderList()
    {
        $api_response = Http::get("http://api.trasuanhatrang.cf/api/order_list");
    ...
    {
        $id = $request->id;
        $api_response = Http::get("http://api.trasuanhatrang.cf/api/order_show/".$id);
        $data = json_decode($api_response);
        $order = $data->data->order;
        $customer = $data->data->customer;
    ...
    function updateOrder(Request $request)
    {
        ...
        CURLOPT_URL => 'http://api.trasuanhatrang.cf/api/order_update/'.$request->id,
    ...
    {
        $id = $request->id;
        $api_response = Http::get("http://api.trasuanhatrang.cf/api/print_order/".$id);
        $data = json_decode($api_response);
        $order = $data->data->order;
        $customer = $data->data->customer;
        $order_detail = $data->data->order_detail;
