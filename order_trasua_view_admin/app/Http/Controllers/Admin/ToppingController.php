...
class ToppingController extends Controller
{
    function listTopping()
    {
        if(Session::has("email")=="admin@gmail.com"){
            $api_response = Http::get("http://api.trasuanhatrang.cf/api/topping_list");
    ...
    function deleteTopping(Request $request)
    {
        if(Session::has("email")=="admin@gmail.com")
        {
            $api_response = Http::post("http://api.trasuanhatrang.cf/api/topping_delete/".$request->id);
    ...
    function saveCreateTopping(Request $request)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://api.trasuanhatrang.cf/api/topping_create',
    ...
    function showTopping(Request $request)
    {
        if(Session::has("email")=="admin@gmail.com"){
            $id = $request->id;
            $api_response = Http::get("http://api.trasuanhatrang.cf/api/topping_show/".$id);
            $data = json_decode($api_response);
            $topping = $data->data->topping;
            $context = [
                "id"=>$id,
    ...
    function updateTopping(Request $request)
    {
        ...
        curl_setopt_array($curl, array(
        CURLOPT_URL => 'http://api.trasuanhatrang.cf/api/topping_update/'.$request->id,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
