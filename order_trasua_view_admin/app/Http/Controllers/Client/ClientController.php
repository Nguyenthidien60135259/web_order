...
class ClientController extends Controller
{
   ...
   {
       $response = Http::get("http://api.trasuanhatrang.cf/api/home");
   ...
   {
       $response = Http::get("http://api.trasuanhatrang.cf/api/menu");
       $data = json_decode($response);
       $products = $data->data->product;
   ...
   {
       $response = Http::get("http://api.trasuanhatrang.cf/api/menu/".$request->id);
       $data = json_decode($response);
       $products = $data->data->product;
       $topping = $data->data->topping;
   ...
   public function postSignUp(Request $request)
   {
    ...
    if ($validator->fails()) {
       ...
       $response = Http::post("http://api.trasuanhatrang.cf/api/postsignUp",[
           'name'=>$request->name,
           'email'=>$request->email,
   ...
   public function postLogin(Request $request){
       $response = Http::post("http://api.trasuanhatrang.cf/api/Login/",[
           'email'=>$request->email,
           'password'=>$request->password,
       ]);
       $data = json_decode($response);
       ...
       if($data->message=="Đăng nhập thành công")
       {
           ...
           return redirect("getLogin")->with("thongbao",$data->message);
       }
   ...
   {
       $response = Http::get("http://api.trasuanhatrang.cf/api/search/".$request->key);
       $data = json_decode($response);
   ...
   public function qlKH(Request $request){
       $id = Auth::user()->id;
       $response = Http::get("http://api.trasuanhatrang.cf/api/qltk/".$id);
       $data = json_decode($response);
       $customer = $data->data->customer;
       $user = $data->data->user;
   ...
   public function changePassWord(Request $request){
       ...
       $response = Http::post("http://api.trasuanhatrang.cf/api/changePassword",[
   ...
   public function changeCus(Request $request)
    {
        $user_id = Auth::user()->id;
        $response = Http::post("http://api.trasuanhatrang.cf/api/changeCus", [
            'address' => $request->address,
            'phone' => $request->phone,
            'user_id' => $user_id
        ]);
   ...
   public function comment(Request $request){
       $response = Http::post("http://api.trasuanhatrang.cf/api/comment",[
