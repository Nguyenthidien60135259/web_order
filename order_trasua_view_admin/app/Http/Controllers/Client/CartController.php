...
class CartController extends Controller
{
	...
	public function addCart($id, Request $request)
	{
		...
		$response = Http::get("http://api.trasuanhatrang.cf/api/addtocart/".$request->id);
	...
	public function getCheckOut(Request $request)
	{
		if (Auth::check()) {
			$cart = unserialize(Cookie::get('item'));
			$id = Auth::user()->id;
			$response = Http::get("http://api.trasuanhatrang.cf/api/checkOut/".$id);
			$data = json_decode($response);
	...
	public function postCheckOut(Request $request)
	{
		...
		])->post("http://api.trasuanhatrang.cf/api/postCheckOut/", [
